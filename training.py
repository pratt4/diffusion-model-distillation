...
def train(data_to_use, savedir, datadir, original_model_dir, devices_to_use, batch_size, use_xla, use_fewer_lsun_examples=False):
    ...
    ema_optimizer = tfa.optimizers.MovingAverage(optimizer, average_decay=average_decay)
    ...
    ema_save_path = os.path.join(savedir, f'{data_to_use}_ema_model.pth')
    ...
    for ep in range(epochs):
        for i, shard in enumerate(tqdm(range(nshards))):
            ...
            if (i + 1) % 80 == 0 or i == nshards - 1:
                ...
                # Assign average weights for EMA
                for p, ema_p in zip(model.parameters(), ema_optimizer.shadow_variables):
                    ema_p.copy_(p)
                ...
                if ema_tl < lowest_ema_loss:
                    ...
                    torch.save(model.state_dict(), ema_save_path)
